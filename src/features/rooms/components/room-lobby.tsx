...
function isGameInProgress(
  status: string,
  metadata: { phase: string; countdownStartAt: number | null }
): boolean {
  if (metadata.phase === "results") return false;
  if (metadata.phase === "lobby" && !metadata.countdownStartAt) return false;
  return status === "playing" || metadata.countdownStartAt !== null;
}
...
export function RoomLobby({
  ...
  const [joinUrl, setJoinUrl] = useState(`/rooms/${code}?game=${encodeURIComponent(game.slug)}`);
  const { players, hostUserId, maxPlayers, status, metadata, error: syncError, refresh } =
    useDuelRoom(code);
  const { join, ready, unready, start, loading, error, playerId } = useRoom(code);
  ...
  const allReady = players.length >= 2 && playersAllReady(players);
  const canStart = isHost && allReady && !loading;
  const playUrl = `/games/${game.slug}/play?room=${encodeURIComponent(code)}`;

  useEffect(() => {
    if (!canStart || typeof document === "undefined") return;
  ...
  useEffect(() => {
    ...
    if (isReadyFromServer === readyPending) {
      setReadyPending(null);
    }
  }, [readyPending, isReadyFromServer]);

  useEffect(() => {
    if (metadata.phase !== "lobby" || metadata.countdownStartAt) return;
    if (status !== "waiting") return;
    setReadyPending(null);
  }, [metadata.phase, metadata.countdownStartAt, status]);
  ...
  async function startGame() {
    ...
    const detail =
      result.data && "detail" in result.data && typeof result.data.detail === "string"
        ? ` (${result.data.detail})`
        : "";
      ...
      {isRegistered ? (
        ...
        <div className="party-card space-y-3 p-4">
          ...
          {!hasJoined && (
            <Button
              className="w-full"
              disabled={loading || !guestNameValid}
              onClick={() => void doJoin()}
            >
              {loading ? t("common.loading") : t("room.enterRoom")}
            </Button>
          )}
        </div>
      )}
      ...
      {(hasJoined || isRegistered) && (
        <>
          ...
          >
            ...
            >
              {isReady ? <Check className="size-5" aria-hidden /> : <Users className="size-5" aria-hidden />}
            </span>
            <div>
          ...
          <div className="party-card p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">{t("room.lobbyHint")}</p>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            {isHost && (
              <Button className="flex-1" disabled={!canStart} onClick={() => void startGame()}>
