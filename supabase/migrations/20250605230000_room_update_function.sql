-- Atualizar salas e estado dos jogadores via security definer (RLS + convidados)
-- Nota: set_room_player_ready e update_game_room foram refinados em 20250605240000_room_players_ready_fix.sql

create or replace function public.get_room_by_id(p_room_id uuid)
returns json
language plpgsql
security definer
...
declare
  v_code text;
begin
  select r.code into v_code from public.rooms r where r.id = p_room_id limit 1;
  if v_code is null then
    return null;
  end if;
  return public.get_room_by_code(v_code);
end;
$$;
...
revoke all on function public.get_room_by_id(uuid) from public;
grant execute on function public.get_room_by_id(uuid) to anon, authenticated;
